...
# Build new sections JSON
new_sections = [section_about(), section_how(), section_services(), section_why(), section_cta()]
new_json = json.dumps(new_sections, ensure_ascii=False)

# Save locally
with open("C:/mcp-agent/new_sections_only.json", "w", encoding="utf-8") as f:
    f.write(new_json)
print(f"New sections JSON: {len(new_json)} bytes, {len(new_sections)} sections")
...
print(f"Current: {len(current_data)} sections")

# Merge current + new
merged = current_data + new_sections
merged_json = json.dumps(merged, ensure_ascii=False)
print(f"Merged: {len(merged)} sections, {len(merged_json)} bytes")

# Upload merged JSON
sftp2 = c.open_sftp()
sftp2.putfo(io.BytesIO(merged_json.encode('utf-8')), '/home/u634372166/merged.json')
sftp2.close()

# PHP update with prepared statement
php_upd = b"""<?php
error_reporting(0);
$db=new mysqli('127.0.0.1','u634372166_Xgj7D','X4JX8XdvZf','u634372166_KpQVV');
$db->set_charset('utf8mb4');
$json=file_get_contents('/home/u634372166/merged.json');
...
$db->query("DELETE FROM wp_options WHERE option_name LIKE '%elementor_css%'");
echo $ok?"SUCCESS rows=$rows":"FAIL:".$db->error;
?>"""
...
res = o2.read().decode('utf-8','ignore')
print(f"Update: {res}")
...
echo 'sections='.count($d).' len='.strlen($row[0]);
?>"""
sftp4 = c.open_sftp()
sftp4.putfo(io.BytesIO(php_ver), '/home/u634372166/v.php')
sftp4.close()
i3,o3,e3 = c.exec_command('php /home/u634372166/v.php 2>&1')
o3.channel.recv_exit_status()
print(f"Verify: {o3.read().decode('utf-8','ignore')}")

# Cleanup
c.exec_command('rm /home/u634372166/g.php /home/u634372166/upd.php /home/u634372166/merged.json /home/u634372166/v.php 2>/dev/null')
c.close()
print("ALL DONE!")
